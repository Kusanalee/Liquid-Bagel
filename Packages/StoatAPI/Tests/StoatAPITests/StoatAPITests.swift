...
final class StoatAPITests: XCTestCase {
    ...
    func testPhase26MemberModerationAndPermissionEndpointRequests() async throws {
        ...
        XCTAssertEqual(fetchedMembers.members.count, 1)
        XCTAssertEqual(fetchedMembers.users.first?.username, "ops")
        XCTAssertEqual(fetchedMembers.diagnostics?.topLevelResponseShape, "object{members,users}")
        XCTAssertEqual(fetchedMembers.diagnostics?.decoderSummary, "decoded members/users wrapper")
        XCTAssertEqual(memberListRequest.httpMethod, "GET")
        XCTAssertEqual(memberListRequest.url?.path, "/servers/server-1/members")
        XCTAssertEqual(URLComponents(url: try XCTUnwrap(memberListRequest.url), resolvingAgainstBaseURL: false)?.queryItems?.first { $0.name == "exclude_offline" }?.value, "false")
