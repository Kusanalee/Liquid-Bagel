...
public enum Phase30ParityMatrixBuilder {
    public static func build() -> ParityMatrix {
        let specs: [(String, String, ParityStatus, String, String, String, String, String, String)] = [
            ...
            ("Core chat", "clipboard paste upload", .partial, "macOS pasteboard, composer flow", "Phase 61 queues pasted image/file clipboard media as visible composer chips while text paste stays normal and upload waits for Send", "Finder/provider edge QA remains pending; explicit drag/drop review remains separate", "Phase 33/61 paste tests", "Paste image and file into the composer, then send with and without text", "Phase 61 live macOS QA"),
            ...
            ("UI/platform", "performance with large channels", .partial, "Lazy timeline and bounded media", "Phase 60 direct lazy items, boxed rows, isolated row state, bounded visible-first preparation, and debounced viewport publication", "Idle and sends enter green; long-channel load/scroll freeze needs post-fix repeat", "Phase 60 timeline tests plus prior perf suites", "Scroll 250 messages, confirm CPU settles below 10%, and sample", "Phase 60 live repeat"),
            ...
            ("UI/platform", "diagnostics", .done, "Developer Verification", "Redacted diagnostics including Phase 60 viewport/row queue/skeleton counters", "Developer-only by design", "Redaction and Phase 60 coalescing tests", "Copy diagnostics", "Keep safe"),
