...
public enum Phase30ParityMatrixBuilder {
    public static func build() -> ParityMatrix {
        let specs: [(String, String, ParityStatus, String, String, String, String, String, String)] = [
            ...
            ("Core chat", "send/edit/delete messages", .partial, "Verified channel message routes", "Send/edit/delete, immediate timeline paint, and Phase 57 full-width row actions", "Live action hit-target and no-flash send confirmation pending", "Message action plus Phase 56/57 tests", "Right-click whitespace and send without channel re-entry", "Phase 57 live re-test"),
            ...
            ("Core chat", "reactions", .partial, "Verified reaction routes", "Phase 59 optimistic add/remove with dedupe and rollback", "Single-account live add/remove/reload and later custom/cross-account QA pending", "Reaction and Phase 59 tests", "Add/remove Unicode reaction and reload", "Phase 59 live re-test"),
            ...
            ("Core chat", "user/avatar hydration", .partial, "Ready/member REST/message/profile data", "Phase 59 adds visible-first priority, bounded warm-up, off-main predecode, and no avatar-driven row rebuild", "Launch avatar timing and idle CPU re-test pending", "Resolver plus Phase 57/59 tests", "Relaunch and inspect visible avatars", "Keep partial until Phase 59 QA"),
            ...
            ("Server/community", "member list", .partial, "Ready members/users and verified member refresh", "Phase 56 retains hydrated members/users while preferring gateway updates", "2,324-member completeness needs live proof", "Member and Phase 56 overlay tests", "Keep large panel open during realtime traffic", "Phase 56 live re-test"),
            ...
            ("UI/platform", "performance with large channels", .partial, "Lazy timeline and bounded media", "Phase 59 change-driven shell, stable scene model ownership, and avatar-isolated presentation", "Post-fix app held 0.0% over 40 seconds; tester interaction repeat remains", "Timeline/media plus Phase 59 tests and full gate", "Repeat sends, scrolling, and settled idle sample", "Phase 59 live interaction re-test"),
            ("UI/platform", "performance with large servers", .partial, "Lazy member list and selected-server hydration", "Phase 59 relationship-only Friends derivation, single-flight shell builds, and stable scene model ownership", "Post-fix app held 0.0% over 40 seconds without prior sampled loops; tester interaction repeat remains", "Member perf plus Phase 59 shell test and final sample", "Exercise 2,324-member server and reconfirm idle", "Phase 59 live interaction re-test"),
