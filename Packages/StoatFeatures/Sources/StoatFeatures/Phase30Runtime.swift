...
public enum Phase30ParityMatrixBuilder {
    public static func build() -> ParityMatrix {
        let specs: [(String, String, ParityStatus, String, String, String, String, String, String)] = [
            ...
            ("Account and session", "user settings sync", .partial, "Verified /sync/settings routes", "Phase 55 syncs allowlisted preferences under a namespaced key; Phase 74 makes fetch/push automatic on connect, on the gateway's UserSettingsUpdate event, on a debounced local edit, and on foreground, with the explicit triad kept behind Developer Options", "Live cross-device QA and official-client key interpretation pending", "Persistence, Phase 55 sync, and Phase 74 automatic-trigger tests", "Edit a preference on one device, confirm it appears on another unprompted", "Phase 74 live audit"),
            ("Account and session", "offline session cache", .partial, "Privacy/scope rule", "Phase 74 persists a versioned, encrypted, per-account-and-environment cache of the server graph, users, unreads, recent message history, and local read state; the shell paints from it on launch before the network is touched, and purges it on sign-out", "Live multi-day dogfood of staleness/reconciliation pending; write-side stays queue-free by design", "Phase 74 persistence, mapper, writer, and startup-routing tests", "Quit online, disable networking, relaunch, confirm content appears immediately", "Phase 74 live audit"),
            ...
            ("Core chat", "emoji picker", .partial, "Native Unicode input, Ready emojis, verified official web token syntax", "Phase 70 keeps friendly search labels while inserting the interoperable :<emoji ULID>: token", "Cross-client artwork/search/send proof and official autocomplete comparison remain pending", "Phase 32-35, 47, 65, 68, and 70 emoji tests", "Send current- and other-known-server emoji and compare Stoat Web", "Phase 70 live re-test"),
            ("Core chat", "custom emoji", .partial, "Ready emojis and verified official web ID-token parser", "Phase 70 resolves official ID tokens globally, legacy name tokens per current server, and preserves the exact matched token through timeline preparation", "Cross-client proof remains pending; reaction behavior and animated playback remain limited", "Model/realtime plus Phase 65/68/70 tests", "Exchange official emoji with Stoat Web and verify legacy/fenced content", "Phase 70 live re-test"),
            ...
            ("Server/community", "member list", .partial, "Ready members/users and verified member refresh", "Phase 69 publishes late identities to the selected server so fallback rows rebuild in place; three cold-launch identity-settling passes succeeded", "Exact native role-rank direction and broader large-server parity remain pending", "Member, Phase 56 overlay, and Phase 69 observation tests", "Keep identities stable while comparing native role ordering", "Role-order live audit"),
            ...
            ("UI/platform", "performance with large servers", .partial, "Lazy member list and selected-server hydration", "Phase 68 server-scopes invalidation; Phase 69 publishes only selected-server identity changes; three cold-launch passes settled correctly", "The identity regression is live-green; role-order and broader large-server parity remain", "Member perf, Phase 59 shell, Phase 68 token, and Phase 69 observation tests", "Keep the member panel stable while auditing role ordering", "Broader large-server audit"),
            ...
            ("UI/platform", "diagnostics", .done, "Developer Verification", "Phase 70 adds adaptive full-label actions and lazy off-main coalesced cached signature validation whose getters read categorical cached state", "Developer-only by design; settled profiling remains a live gate", "Redaction and Phase 60/68/69/70 diagnostics tests", "Check minimum-width labels, copy diagnostics, and profile 30 settled seconds", "Keep safe"),

            ("Core chat", "voice", .partial, "LiveKit-backed VoiceEngine, join_call route, gateway-synced roster", "Phase 75 adds join/leave, mute/deafen, push-to-talk (in-app-focused), device selection, and a live participant roster with speaking indicator", "join_call response shape and node selection unverified against a live server; no automated audio-path test possible in this environment", "Phase 75 permission/preference/roster tests", "Join a voice channel with two accounts and confirm mute/deafen/push-to-talk audibly work", "Phase 75 live QA"),
            ...
            ("Core chat", "screen share", .partial, "LiveKit macOS display/window capture", "Native source picker and video-only publication", "Signed-in capture and remote rendering QA pending", "Phase 76 media tests", "Display/window two-client pass", "Phase 76 live QA"),
