...
| Core chat | emoji picker | partial | Native Unicode input, Ready emojis | Composer popover visibly opens, inserts common Unicode emoji, and includes known server emoji shortcodes | Full custom autocomplete and exact official syntax deferred | Phase 32/33/34 emoji tests | Insert/react emoji | Custom emoji QA |
| Core chat | custom emoji | partial | Ready emojis | Models decode emojis; picker/reaction display resolves known emoji via `emojis` media tag; known `:name:` content tokens can render through bounded image loading | Exact message-content syntax/live media QA still pending | Model/realtime/Phase 33/34 tests | Emoji server | Live syntax QA |
...
| Core chat | user/avatar hydration | partial | Ready users/members, message users, relationship/profile data | Central resolver prevents full raw-ID author names, member nicknames win, bot metadata preserved, avatar metadata remains bounded, missing avatars do not drop rows | Live QA must confirm names/avatars in real chat | Phase 31/32/33/34 resolver tests | Inspect affected chats | Keep partial until live QA |
...
| Server/community | member list | partial | Ready members/users, explicit `GET /servers/{id}/members` refresh | Missing/offline fallbacks, role grouping, role-colored names, profile entry, missing-user/avatar diagnostics, manual Developer Settings refresh | Live large-server completeness needs dogfood proof; Ready may not include members absent from server payload | Member + Phase 33/34 tests | Large server | Keep partial until live QA |
...
| UI/platform | performance with large servers | partial | Lazy member list | Member diagnostics include known/rendered/missing/avatar/dropped counts and context source | More live QA needed | Member perf tests | Large server | Monitor |
