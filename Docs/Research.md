...
- Backend source: `stoatchat/stoatchat` at `main` (fetched via `gh api repos/stoatchat/stoatchat/contents/...`), specifically `crates/core/parser/src/lib.rs`, `crates/delta/src/routes/channels/message_send.rs`, `crates/delta/src/routes/channels/group_add_member.rs`, `crates/delta/src/routes/channels/group_remove_member.rs`, `crates/delta/src/routes/channels/channel_delete.rs`, and `crates/core/permissions/src/models/channel.rs`.
- Generated API schema: `stoatchat/javascript-client-api` `src/schema.ts` and `src/routes.ts` (the `DataMessageSend` and `DataCreateGroup` request bodies, and the recipient routes).
- Official web client: `stoatchat/for-web` `packages/client/components/keybinds/keybindActions.ts` and `keybindSequences.ts`.

### Verified mention token syntax

- The backend lexes message content with a `logos`-based tokenizer (`crates/core/parser/src/lib.rs`) that recognizes, outside fenced/inline code spans: `<@ULID>` (user mention), `<%ULID>` (role mention), `<#ULID>` (channel mention), `:ULID:` (custom emoji by ID), plus the bare keyword tokens `@everyone` and `@online`. A backslash (`\`) escapes the following token. Content inside a code span (`` ` ``/`` `` ``/`` ``` ``, matched and closed by the same marker) is not tokenized at all, so mentions/emoji typed inside code stay fully literal — this matches the sanitization boundary Liquid Bagel already uses for custom emoji and must reuse for mention tokenization.
- `DataMessageSend` (`schema.ts`) has **no `mentions` field**. The backend derives `Message.mentions`/`role_mentions` by running this parser over `content` server-side after the message is created (confirmed via `message_send.rs`, which reads `message.mentions`/`message.role_mentions` back off the freshly created message rather than accepting them as input). **Conclusion: no `MessageDraft`/API client change is needed for Part A5** — sending the raw `<@ULID>` token in `content` is sufficient, and the server-parsed `mentions` on the resulting/echoed message is what notification/badge classification should keep using.
- `ReplyIntent.mention: bool` (on `replies[]`) is a separate flag: it does not affect content parsing, it tells the backend to also add the reply target as a mention (badge/notification) even if the composed content contains no `<@ULID>` token for them.
- Channel mentions (`<#ULID>`) and role mentions (`<%ULID>`) exist as authored content tokens (not request-body fields), same tier as user mentions. Phase 71 adds composer autocomplete for both: channels reuse the Ready-scoped sidebar/navigation visibility predicate and are limited to server text channels; roles include every server role because the modeled `Role` has no `mentionable` flag. There is no `ViewChannel` permission evaluator in the client, so this is not claimed as permission evaluation.
- Phase 71 also adds custom-emoji `:shortcode` typeahead backed by the revision-cached Phase 68 Ready emoji index. Picker and typeahead both insert the exact `:<emoji ULID>:` token. Unicode `:name:` typeahead remains deliberately unsupported because the repository has no verified Unicode alias table; a partial hand-authored table would create inconsistent behavior.
- No official-client row-level self-mention highlight styling was found in the fetched frontend sources; Part A3's accent styling should follow Liquid Bagel's own design system (existing search-highlight seam) rather than a verified color spec.

### Verified group recipient route semantics

- **Add** (`PUT /channels/{group}/recipients/{member}`, `group_add_member.rs`): gated by the `InviteOthers` **channel permission**, not ownership. Group DM channels use `DEFAULT_PERMISSION_DIRECT_MESSAGE` (`crates/core/permissions/src/models/channel.rs`), which includes `InviteOthers` from the shared `DEFAULT_PERMISSION` set — so **any existing group member can add a new recipient**, provided the adder and the target are friends (`NotFriends` error otherwise). Already-in-group returns 409 Conflict. Bots cannot call this route (`IsBot`). Success emits `ChannelCreate` to the newly added user, `ChannelGroupJoin` to the group, and a `UserAdded` system message.
  - **Plan correction**: B2's original owner-only gating assumption applies to *remove*, not *add*. Add should be available to any current group member (subject to the friends-with-target check), matching the backend's actual authorization.
- **Remove** (`DELETE /channels/{group}/recipients/{member}`, `group_remove_member.rs`): **owner-only** (`user.id != owner` → `MissingPermission(ManageChannel)`). Removing yourself via this route is explicitly rejected (`CannotRemoveYourself`) — leaving a group one is a member of is a distinct, already-existing flow through `DELETE /channels/{target}` (`channel_delete.rs`, "Deletes a server channel, leaves a group or closes a group"), not the recipients route. Target must currently be in the group (`NotInGroup` otherwise). Success also handles voice-channel cleanup for the removed user.
  - **Plan correction**: B2's "Remove from Group" action must only ever be offered for other members by the group owner; self-removal must continue to route through the existing leave/close-group path, not the new remove-recipient API.
- Both routes produce `ChannelGroupJoin`/`ChannelGroupLeave` gateway events that `RealtimeStateStore` already reconciles (confirmed at implementation time against `RealtimeStateStore.swift:310-318`).

### Verified official-client keybind table (macOS)

- Source: `packages/client/components/keybinds/keybindSequences.ts`, `DEFAULT_MAC_SEQUENCES`.
- `CHAT_JUMP_END` = **Escape** — marks the channel as read, jumps to the end of the conversation, and focuses composition. This is the verified candidate for Part D2's "mark channel read" shortcut.
- `CHAT_MARK_SERVER_AS_READ` = **Shift+Escape** — marks the entire current server as read. Verified candidate for a new "mark server as read" command.
- `NAVIGATION_CHANNEL_UP`/`DOWN` = **Command+ArrowUp/Down** (single modifier); `NAVIGATION_SERVER_UP`/`DOWN` = **Control+Command+ArrowUp/Down**. Phase 71 remaps Liquid Bagel to this verified table and removes the separate Command-Down shortcut from the Jump to Newest menu item because Escape already implements `CHAT_JUMP_END`. Menu key equivalents take precedence over the responder chain, so Command-Up/Down no longer move to the beginning/end of the composer document; the official client claims these combinations globally too.
- `CHAT_FOCUS_COMPOSITION` triggers on any plain printable keypress (typing-to-focus); `CHAT_CANCEL_EDITING`/`CHAT_REMOVE_COMPOSITION_ELEMENT`/`CLOSE_MODAL`/`CLOSE_FLOATING`/`CLOSE_SIDEBAR` all also bind to Escape with a documented priority order. No other officially-bound actions were found beyond this table (the `Keybinds.ts` store's `DEFAULT_VALUES` table with additional candidate actions like autocomplete up/down/select and message-edit-previous is present only as commented-out/inactive legacy code, not a shipped mapping — not used as a verification source).
- No unverified candidates from the original plan (mute toggle, unread-channel navigation) were found in official sources; they are **not** added this phase.
