...
- Channel mentions (`<#ULID>`) and role mentions (`<%ULID>`) exist as authored content tokens (not request-body fields), same tier as user mentions. Rendering them is in scope for Part A1 alongside user mentions since the tokenizer change is shared; composer autocomplete for channels/roles is not required this phase (only `@user` autocomplete was planned).
...
- `NAVIGATION_CHANNEL_UP`/`DOWN` = **Command+ArrowUp/Down** (single modifier); `NAVIGATION_SERVER_UP`/`DOWN` = **Control+Command+ArrowUp/Down**. Liquid Bagel's existing bindings are inverted relative to this (`Cmd-Control-↑/↓` for channel, `Cmd-Option-↑/↓` for server, per `App/AppCommands.swift`). This is a genuine parity mismatch, but remapping already-shipped navigation shortcuts is a bigger behavior change than "fill a gap" — Phase 58 documents this as a **known, deferred deviation** rather than silently remapping existing muscle-memory shortcuts.
