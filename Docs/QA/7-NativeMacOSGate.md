...
| 6 | Run a VoiceOver pass: settings tabs, quick switcher, server rail, channel sidebar, timeline rows (including mentions), composer (including the `@` autocomplete popover), member panel, badges | Elements have sensible labels; unread/mention badges announce counts; **the message body announces each mention inline as "mention, `<name>`" / "mentions you" / "channel mention, `<name>`" / "role mention, `<name>`"** (Phase 72: mentions are runs in one Text, so they are not separate elements); the autocomplete popover announces "autocomplete suggestions, N results" with the highlighted candidate marked selected; no unlabeled controls | | |
...
| 10 | Check role-color readability in both light/dark and Increase Contrast modes, including mentions and embed cards | Sanitized colors keep contrast; mentions render as a tinted rectangular run (Phase 72) whose role color is preserved; Increase Contrast falls back to the system accent same as author names; embed cards honor Increase Contrast and Reduce Transparency; fallbacks diagnosed, not broken | | |
