...
- `join_call`'s actual response shape (is `token` the right key? does it include a URL, or must the client pick a node from `StoatConfig.features.livekit.nodes`?) is unverified. `VoiceJoinResponse` is written defensively (optional `url`) but the exact wire contract needs confirming against a live Revolt-compatible server before shipping.
