...
Phase 73 removed the mock runtime from the shipping product: test doubles live only in test targets, and `Scripts/check.sh` fails if one reappears in a library source. Real credentials are required to see any content. On first launch with no saved credential the app shows a native login card (`FirstRunLoginView`) instead of the shell. If a credential was saved from a previous session the app auto-validates and connects; the shell appears only after the realtime Ready event. Session tokens are stored only in scoped Keychain; nothing credential-adjacent is logged or sent to diagnostics. Test doubles remain available to the package test targets only; SwiftUI previews were removed with the mock data they depended on. Persistent message/media cache, voice/video, APNs/background push, and automatic background networking remain deferred. Settings cloud sync is explicit fetch/push only.
